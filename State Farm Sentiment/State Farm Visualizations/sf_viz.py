...
#explode = (0, 0.1, 0, 0)  # only "explode" the 2nd slice (i.e. 'Hogs')
    ...
    plt.title("State Farm Sentiment Breakdown", fontname = "Arial", fontsize = 15)
    plt.savefig('sfbreak.png', transparent=True, dpi = 100)
    plt.show()
...
def over_time(df):
    ...
    plt.title("State Farm Sentiment Over Time", fontname = "Arial", fontsize = 15 )
    plt.ylabel("Number of Tweets", fontsize = 10)
    plt.xlabel("Month", fontsize = 10)
    plt.legend()
    plt.tight_layout()
    plt.savefig('sfovertime.png', transparent=True, dpi = 100)
    plt.show()
...
def mean_plot(df):
    ...
    plt.title("State Farm Sentiment Mean vs Month", fontname = "Arial", fontsize = 15)
    plt.xlabel('Month', fontsize = 10)
    plt.ylabel('Compound Sentiment Score / Mean', fontsize = 10)
    plt.legend()
    plt.tight_layout()
    plt.savefig("sfmean.png", transparent=True, dpi = 100)
    plt.show()
    

def main():
    df = read_file("statefarm_compound.csv")
    pie_count(df)
    over_time(df)
    mean_plot(df)
